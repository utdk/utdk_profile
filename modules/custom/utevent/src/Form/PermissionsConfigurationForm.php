  ...
  public function submitForm(&$form, FormStateInterface $form_state) {
    ...
    if (in_array($manager_role, $allowed_roles)) {
      if (Permissions::assignPermissions('manager', $manager_role)) {
        $this->messenger->addMessage($this->t('Management permissions for the Event add-on have been added to the %role role.', ['%role' => $manager_role]));
      }
    }
    ...
    if (in_array($editor_role, $allowed_roles)) {
      if (Permissions::assignPermissions('editor', $editor_role)) {
        $this->messenger->addMessage($this->t('Content editing permissions for the Event add-on have been added to the %role role.', ['%role' => $editor_role]));
      }
    }
  }
