...
use Drupal\Core\Url;
/**
 * Helper class to get information from a site's utexas.extend.yml file.
 */
class Extender {
  /**
  ...
  public function getInfo() {
    // Discover utexas.extend.yml first in the `sitePath`, and then defer to
    // `sites/all`.
    $paths[] = $this->sitePath . '/utexas.extend.yml';
    $paths[] = $this->root . '/sites/all/utexas.extend.yml';
  ...
  /**
   * Returns the list of UTexas Extensions to enable.
   *
   * @return string[]
   *   The modules to enable.
   */
  public function getUTexasExtensions() {
    $info = $this->getInfo();
    // Return FALSE instead of empty array because empty array means "don't
    // enable _any_ extensions" in this case.
    return isset($info['utexas_extensions']) ? $info['utexas_extensions'] : FALSE;
  }
  /**
   * Returns the list of UTexas sub-components that should NOT be installed.
   *
   * @return string[]
